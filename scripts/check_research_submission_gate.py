...
This is a reproducibility/claim-safety gate, not a statistical proof of novelty.
It verifies that every required experiment artifact exists, is explicitly marked
as simulation evidence, contains finite machine-readable results, and exposes
key fields needed for the paper claims.
"""
...
def _finite_tree(value: Any, path: str = "results") -> list[str]:
    ...
    if isinstance(value, float) and not math.isfinite(value):
        ...
        for key, item in value.items():
            errors.extend(_finite_tree(item, f"{path}.{key}"))
    elif isinstance(value, list):
        for index, item in enumerate(value):
            errors.extend(_finite_tree(item, f"{path}[{index}]"))
    return errors
...
def validate_artifact(name: str, payload: dict) -> list[str]:
    ...
    errors.extend(f"{name}: {x}" for x in _finite_tree(results))
    ...
    if name == "full-metrics":
        ...
        summary = results.get("summary", {})
        _require(bool(summary), f"{name}: empty calibration summary", errors)
        for draws, row in summary.items():
            for key in ("false_feasible_rate", "false_infeasible_rate", "decision_disagreement_rate", "selected_action_disagreement_rate"):
                val = row.get(key)
                _require(isinstance(val, (int, float)) and 0.0 <= val <= 1.0, f"{name}: invalid {key} at draws={draws}", errors)
        ...
        _require(bool(cells), f"{name}: empty physical map", errors)
        _require("derived_profile_limits" in results, f"{name}: missing derived profile limits", errors)

    elif name == "mismatch":
        _require(bool(results.get("summary")), f"{name}: empty mismatch summary", errors)

    elif name == "runtime":
        summary = results.get("summary", {})
        _require(bool(summary), f"{name}: empty runtime summary", errors)
        for draws, policies in summary.items():
            if "B4_ROBUST_JOINT" in policies:
                ...
                for key in ("median_us", "p95_us", "p99_us"):
                    _require(b4.get(key, 0) > 0, f"{name}: invalid B4 {key} at draws={draws}", errors)

    elif name == "confidence-maps":
        _require(bool(results), f"{name}: empty confidence maps", errors)

    elif name in ("uncertainty", "stress", "ablations", "reliability-targets", "uncertainty-sources"):
        _require(bool(results), f"{name}: empty results", errors)

    elif name == "pareto":
        _require("physical_points" in results, f"{name}: missing physical_points", errors)

    return errors


def run_gate(root: Path) -> dict:
    report: dict[str, Any] = {"input_dir": str(root), "artifacts": {}, "errors": []}
    for name in REQUIRED:
        path = root / f"{name}.json"
        if not path.exists():
            report["errors"].append(f"{name}: missing artifact {path}")
            report["artifacts"][name] = {"status": "MISSING"}
            continue
        try:
            payload = json.loads(path.read_text())
        except Exception as exc:
            report["errors"].append(f"{name}: unreadable JSON: {exc}")
            report["artifacts"][name] = {"status": "INVALID_JSON"}
            continue
        errors = validate_artifact(name, payload)
        report["artifacts"][name] = {"status": "PASS" if not errors else "FAIL", "errors": errors}
...
def main() -> None:
    args = parse_args()
    report = run_gate(Path(args.input_dir))

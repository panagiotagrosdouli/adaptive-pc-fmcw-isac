...
| Policy abstention is not identical to hard physical infeasibility | v2.1 selector + post-freeze audit fields | `any_physics_feasible_action` and `selection_status` distinguish the two cases in new records | A policy may abstain even when at least one action is physically admissible | Every unselected state is physically infeasible |

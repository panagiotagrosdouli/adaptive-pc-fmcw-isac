...
% Evidence basis:
% - frozen main benchmark: publication-v2.1 frozen evidence
% - reviewer-grade supplemental evidence: publication-v2.1 supplemental bundle
% All numerical values are simulation/analytical or host-runtime outputs, not hardware measurements.
...
The hindsight Oracle selects 66.67\% of cases and has conditional joint QoS equal to one because it uses true receiver-level state information unavailable to deployable policies. It is therefore reported only as a non-deployable reference.
...
The most extreme ablation is the removal of the physical feasibility gate. In this frozen supplemental design, the ungated variant selects 31.67\% of states but achieves 0\% joint QoS. This does not imply that every conceivable ungated controller must fail. Rather, it demonstrates that optimization over candidate PHY profiles is scientifically ill-posed in this evaluated design if actions that violate hard range or unambiguous-velocity limits are allowed to compete with physically admissible actions.
...
\subsection{Finite-draw calibration and runtime limitation}

The reviewer-grade supplemental run explicitly calibrates the finite Monte-Carlo decision rule used by B4 against a nested 4096-draw reference. At 32 robust draws the declared 0.95 confidence-qualified reliability target is structurally unattainable even with 32/32 successful samples. At 64, 128, 256, and 512 draws the observed \emph{feasibility-decision} disagreement rates relative to the reference are 0.583\%, 0.333\%, 0.083\%, and 0\%, respectively, on the tested 1,200-state calibration bank. Exact action choice converges more slowly: selected-action disagreement remains 3.583\%, 2.167\%, 1.250\%, and 0.833\% at 64, 128, 256, and 512 draws, respectively. Therefore the 512-draw result must not be described as identical action selection. Moreover, the 4096-draw reference is a low-Monte-Carlo-noise comparator, not physical ground truth.

The same supplemental evidence quantifies the computational price. B4 median host decision latency is approximately 32.17~ms at 64 draws, 63.81~ms at 128 draws, 127.19~ms at 256 draws, and 251.04~ms at the frozen 512-draw setting. The corresponding 95th-percentile values are approximately 64.37, 127.98, 255.28, and 504.11~ms. These are measurements of the unoptimized Python/Linux host implementation, not embedded-target or RF-prototype timing.

Accordingly, no hard real-time deployment claim is made. The timing result instead motivates algorithmic optimization through action pruning after the physical gate, vectorized uncertainty evaluation, parallel sampling, cached surrogate models, and reduced-draw or sequential confidence tests. The physical gate is especially useful here because it can remove impossible candidates before the expensive robust reliability evaluation.
...
The supplemental full-metric bank independently reproduces the same direction of effect: B4 selects 12.50\% of 1,200 paired states with 100\% observed conditional joint QoS and a 98.23\% Wilson lower bound, whereas B3 selects 20.00\% with 80.83\% conditional joint QoS. However, B4 has lower unconditional joint QoS than B3 by $-0.03667$, with a 95\% paired-bootstrap interval of $[-0.04833,-0.02583]$. Distribution-shift banks likewise retain negative B4--B3 unconditional differences, so universal or unconditional superiority is explicitly rejected.

The evidence does not support the claims that B4 maximizes unconditional joint QoS, remains reliable under arbitrary SNR or interference mismatch, provides global Pareto optimality outside realized selected operating points, or is already real-time. All reported numerical results are simulation, analytical, statistical, or host-runtime evidence; none are adaptive-controller hardware measurements. The principal contribution is therefore the explicit characterization of the physically feasible and statistically reliable operating region, together with the demonstrated reliability--availability--computation trade-off induced by robust PC-FMCW PHY adaptation.

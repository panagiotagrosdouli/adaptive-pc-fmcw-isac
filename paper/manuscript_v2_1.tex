...
\end{equation}
where $\mu$ is the programmed chirp slope, $P_m$ is transmit power, and $\phi_m(t)$ is the phase-coded communication sequence. For an ideal linear chirp that sweeps exactly $B$ over an active ramp $T_c$, $\mu=B/T_c$; source-defined profiles may instead provide $B$, $\mu$, and the ADC-valid interval as distinct quantities. Multiple phase-code chips may be carried in one chirp.
...
The source-grounded short-range profile follows a Texas Instruments automated-parking reference profile \cite{tiParking}: \SI{77}{GHz} carrier, \SI{858}{MHz} valid sweep bandwidth, \SI{40}{MHz/\micro s} programmed chirp slope, a \SI{25.6}{\micro s} ADC/chirp capture interval, $T_r=\SI{115.8}{\micro s}$, $F_s=\SI{10}{MSPS}$, 256 samples/chirp, and 64 chirps/frame. Because the source reports valid sweep bandwidth and programmed slope separately, the simulator does not infer the slope as $858\,\mathrm{MHz}/25.6\,\mu\mathrm{s}$. The resulting range resolution is approximately \SI{0.175}{m}, positive-IF range support is approximately \SI{18.74}{m}, and monostatic unambiguous radial-velocity support is approximately \SI{8.41}{m/s}. A separate high-mobility capability profile uses a \SI{1}{GHz} sweep and \SI{20}{\micro s} ramp with \SI{37.5}{MSPS} sampling, giving approximately \SI{56.21}{m} positive-IF range support and \SI{48.67}{m/s} unambiguous radial-velocity support. The latter is a composite capability reference, not a claimed commercial preset.

...
% - reviewer-grade supplemental evidence: GitHub Actions run 34696063382
...
The source-grounded profile limits make this distinction explicit. The short-range parking profile uses the TI-programmed \SI{40}{MHz/\micro s} chirp slope and therefore supports approximately 18.74~m positive-IF range at \SI{10}{MSPS}, together with 8.41~m/s unambiguous radial velocity; its 858~MHz valid sweep gives approximately 0.175~m range resolution. The high-mobility capability profile supports approximately 56.21~m and 48.67~m/s. The resulting range--velocity map therefore contains three physically distinct regions: no feasible profile, high-mobility-only feasibility, and overlap where both profiles are admissible. Policy abstention should not be conflated with this hard physical infeasibility.
...
The reviewer-grade supplemental run explicitly calibrates the finite Monte-Carlo decision rule used by B4 against a 4096-draw reference. At 32 robust draws the declared confidence target is not attainable under the acceptance rule. At 64, 128, 256, and 512 draws the observed decision-disagreement rates are 0.583\%, 0.333\%, 0.083\%, and 0\%, respectively, on the tested calibration bank. These are empirical finite-bank calibration results, not mathematical guarantees that disagreement is zero outside the evaluated state distribution.

The same supplemental run confirms that the current Python reference implementation is not a real-time vehicular controller at the frozen 256-draw B4 setting. B4 median decision latency increases from 36.29~ms at 64 draws to 70.47~ms at 128 draws, 138.89~ms at 256 draws, and 272.81~ms at 512 draws; the corresponding 95th-percentile latencies are 73.03, 142.13, 279.71, and 549.16~ms. These are prototype timings on GitHub Actions infrastructure, not embedded-target measurements.
...
The evidence does not support the claims that B4 maximizes unconditional joint QoS, remains reliable under arbitrary SNR or interference mismatch, provides global Pareto optimality outside realized selected operating points, or is already real-time. All reported numerical results are simulation or analytical evidence; none are hardware measurements. The principal contribution is therefore the explicit characterization of the physically feasible and statistically reliable operating region, together with the demonstrated reliability--availability--complexity trade-off induced by robust PC-FMCW PHY adaptation.

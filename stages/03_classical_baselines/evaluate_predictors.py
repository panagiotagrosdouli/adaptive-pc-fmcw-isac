...
def link_metrics(model,pred_relative,true_relative,dt=.1):
 rp,bp=range_bearing(pred_relative); rt,bt=range_bearing(true_relative); ps=[model.evaluate(range_m=float(r),pointing_error_rad=float(abs(b))) for r,b in zip(rp,bp)]; ts=[model.evaluate(range_m=float(r),pointing_error_rad=float(abs(b))) for r,b in zip(rt,bt)]
 ...
 def life(o): i=np.flatnonzero(o); return float((i[0]+1)*dt) if len(i) else float(len(o)*dt)
...
def main():
 ...
 print(json.dumps({"rows":len(rows),"scenarios":len(set(S)),"split":a.split,"predictors":a.predictors,"link_metrics":model is not None,"true_sdc_future_geometry":bool(model)},indent=2)); return 0
